<template>
  ...
  <v-card flat class="gp">
    ...
    <!-- Plugin contributions (`editExtension`, target 'group'). Extra keys
...
/* Footer buttons now come from <LjButton>; the surface tokens (--font,
   --radius-sm, --ink-2, --border) cascade from the enclosing LjDialog card's
   `.lj-surface`. Only the transparent panel background, field refinements,
   and the footer layout remain. */
.gp {
  background: transparent !important;
}
.gp :deep(.v-card-text) { padding: 4px 24px 4px !important; }
/* Clean rounded fields (match the user dialog). */
.gp :deep(.v-field) { border-radius: var(--radius-sm, var(--lj-radius-sm, 12px)); font-family: var(--font, "Bricolage Grotesque", system-ui, sans-serif); }
.gp :deep(.v-field__prepend-inner .v-icon) { opacity: .55; }
.gp :deep(.v-label) { font-weight: 600; }

/* Footer layout (delete left, cancel/save right). */
