...
// ../src/main/resources/.../webjars/id/vue/index.js is what the host
// loads at runtime — here we test the authoring surface directly.
import pluginIdDef from '../index.js'
...
describe('plugin-id contract', () => {
  ...
  it('install() registers all /id/* routes on the given router', () => {
    ...
    expect(registered).toEqual(expect.arrayContaining([
      ...
      '/id/delegate',
      // canonical scheme: container scopes live at /id/scope (legacy
...
describe('callFeature()', () => {
  ...
  it('throws when the plugin has no feature function', () => {
    pluginRegistry.register('noop', { id: 'noop', install: () => {} })
    expect(() => callFeature('noop', 'x')).toThrow(/does not expose a feature/)
    pluginRegistry.remove('noop')
  })
})
