...
public class PasswordResourceTest extends AbstractAppTest {
	...
	private PasswordResource newResource() {
		...
		Mockito.when(resource.servicePluginLocator.getResource("service:mail:smtp:local", MailServicePlugin.class)).thenReturn(mailServicePlugin);
		Mockito.when(mailServicePlugin.send(ArgumentMatchers.eq("service:mail:smtp:local"), ArgumentMatchers.any(MimeMessagePreparator.class)))
				.thenAnswer(a -> {
	...
	public void sendMailPasswordNoPassword() throws MessagingException {
		...
		final MailServicePlugin mailServicePlugin = resource.servicePluginLocator.getResource("service:mail:smtp:local", MailServicePlugin.class);
		Mockito.when(mailServicePlugin.send(ArgumentMatchers.eq("service:mail:smtp:local"), ArgumentMatchers.any(MimeMessagePreparator.class)))
				.thenAnswer(
	...
	public void requestRecovery() throws MessagingException {
		...
		Mockito.verify(message, Mockito.atLeastOnce())
				.setContent("First Last-<a href=\"host#reset=" + passwordReset.getToken() + "/fdaugan\">host#reset=" + passwordReset.getToken()
						+ "/fdaugan</a>-First Last-<a href=\"host#reset=" + passwordReset.getToken() + "/fdaugan\">host#reset="
